...
\begin{tikzpicture}[
    ...
    \node[veh] (veh) {Vehículo\\OBD-II / CAN};
    \node[esp, below=of veh] (esp) {ESP32-S3\\Firmware AutoSense\\\footnotesize adquisición, guardia de solo lectura, compactación};
    \node[app, below=of esp] (app) {Aplicación móvil\\React Native / Expo\\\footnotesize sesión, telemetría y navegación};
    \node[cloud, below=of app] (cloud) {Firebase\\Auth, Firestore, Functions y Storage};

    \node[store, left=18mm of esp] (lfs) {LittleFS\\\footnotesize perfil activo\\historial binario};
    \node[serial, right=18mm of esp] (usb) {USB-CDC\\dashboard local\\monitor serial};
...
\endgroup
\caption{Arquitectura lógica de AutoSense y relación entre sus capas principales.}
\label{fig:arquitectura-autosense}
\end{figure}

La figura resume la separación funcional del sistema. El vehículo genera la información bruta; el ESP32-S3 la recibe, la valida y la compacta; la app la consume y la presenta; Firebase administra el soporte de sesión y perfiles; y LittleFS conserva el estado local. El flujo de red no pasa por el microcontrolador como un enlace Wi-Fi independiente, sino por el teléfono, que actúa como puente entre la comunicación BLE y los servicios en la nube.
...
Firebase cubre el plano de control y no el plano de adquisición en tiempo real. Authentication mantiene el acceso seguro, Firestore conserva documentos de usuario, vehículo y actividad, Functions resuelve el VIN y el perfil, y Storage hospeda los archivos de perfil descargables. De esta manera, la nube actúa como centro de identidad y distribución de configuración, mientras que la telemetría de alta frecuencia permanece en el firmware y en la app. El resultado es una arquitectura dividida en responsabilidades claras: el vehículo produce, el ESP32-S3 procesa y conserva, la app presenta y sincroniza, y la nube administra el contexto del sistema.
...
En conjunto, esta arquitectura permite que AutoSense funcione con un nivel de desacoplamiento suficiente para evolucionar cada bloque sin romper el resto. El firmware puede cambiar su plan de consulta sin alterar la interfaz móvil; la app puede actualizar su navegación sin modificar la lógica CAN; y la nube puede ampliar el catálogo de perfiles sin tocar la política de solo lectura del ESP32-S3. Esa separación es la base sobre la que se desarrollan las subsecciones siguientes del capítulo.
...
\begin{tikzpicture}[
    ...
    \node[boot] (start) {Arranque del ESP32-S3\\\scriptsize Serial 115200, LED, CAN RX/TX y servicios base};
    \node[config, below=of start] (cfg) {Configuración y perfil\\\scriptsize \texttt{/config.ini}, \texttt{/active\_profile.json}, intervalos y perfil \texttt{vw\_passat\_2016}};
    \node[safety, below=of cfg] (guard) {Guardia de solo lectura\\\scriptsize permite OBD 01/02/03/06/07/09/0A y bloquea borrado, control o escritura};
    \node[data, below=of guard] (obd) {Adquisición OBD-II por CAN\\\scriptsize discovery de PIDs, consultas Mode 01 y servicios diagnósticos seguros};
    \node[data, below=of obd] (sample) {Muestra compacta\\\scriptsize \texttt{ObdCompactSample}, \texttt{validMask}, edad máxima de muestra y decodificación};
    \node[io, below left=8mm and 11mm of sample] (ble) {BLE GATT\\\scriptsize comandos, perfil y telemetría};
    \node[io, below=8mm of sample] (usb) {USB-CDC\\\scriptsize dashboard y depuración};
    \node[ai, below right=8mm and 11mm of sample] (logic) {Ruta e IA local\\\scriptsize city/highway, Z-score e Isolation Forest};
    \node[config, below=18mm of usb] (log) {Persistencia local\\\scriptsize log circular en \texttt{obdlog}, baseline y modelo en flash};
...
\endgroup
\caption{Flujo interno del firmware AutoSense desde el arranque hasta la persistencia y salida de datos.}
...
\begin{tikzpicture}[
    ...
    \node[a] (tick) {\texttt{OBD.tick()}\\planificación de consultas OBD-II};
    \node[b, below=of tick] (bletick) {\texttt{BLE\_PROTO.tick()}\\comandos y telemetría};
    \node[c, below=of bletick] (vag) {\texttt{VAG.tick()}\\UDS seguro si el perfil lo permite};
    \node[a, below=of vag] (can) {Procesamiento CAN\\respuestas OBD y VAG};
    \node[e, below=of can] (sample) {Muestra compacta\\\texttt{ObdCompactSample} + \texttt{validMask}};

    \node[c, below left=9mm and 9mm of sample] (logger) {\texttt{LOGGER.tick()}\\historial binario};
    \node[d, below=9mm of sample] (analysis) {Ruta + anomalía\\contexto, score y severidad};
    \node[e, below right=9mm and 9mm of sample] (dash) {\texttt{DASH.tick()}\\dashboard USB-CDC};
    \node[f, below=13mm of analysis] (next) {\texttt{delay(1)}\\siguiente iteración};
...
\endgroup
\caption{Ciclo lógico de ejecución de \texttt{appLoop()} en el firmware AutoSense.}
...
El soporte VW/VAG extendido se mantiene bajo dos condiciones simultáneas: la configuración VAG debe estar habilitada y el perfil activo debe contener permisos extendidos de solo lectura. En consecuencia, el ESP32-S3 no activa el escaneo extendido solo por detectar un VIN de Volkswagen. Esta decisión evita consultas ampliadas sin autorización explícita del perfil y mantiene el diseño coherente con el principio de lectura segura.
...
Perfil vehicular & Resolución, descarga y aplicación de perfiles OBD2. & Permite usar \texttt{vw\_passat\_2016}, perfil genérico o modo demo. \\
...
\vspace{2mm}
{\footnotesize Emparejamiento OBD2}
\end{minipage}
\hfill
\begin{minipage}{0.40\textwidth}
\centering
...
\node[box, fill=gray!12, below=0.65cm of localTitle] (obd) {OBD-II / CAN\\lecturas del vehículo};
\node[box, fill=blue!10, below=of obd] (sample) {\texttt{ObdCompactSample}\\muestra validada};
\node[box, fill=orange!15, below=of sample] (log) {\texttt{obdlog}\\historial binario};
\node[box, fill=yellow!16, left=1.2cm of sample] (config) {\texttt{configfs}\\LittleFS\\configuración y perfil};

\node[box, fill=green!12, below=0.65cm of cloudTitle] (ble) {BLE\\telemetría en vivo};
\node[box, fill=purple!10, below=of ble] (app) {Aplicación móvil\\caché y sesión};
\node[box, fill=cyan!12, below=of app] (cloud) {Firebase\\Firestore / Storage};
...
Aplicación móvil & FileSystem de Expo & Caché del perfil vehicular activo & Reutilizar perfiles sin descargarlos en cada uso \\
Firebase & Firestore y Storage & Usuario, vehículo, viajes, alertas, dashboard y perfiles & Sincronización remota y disponibilidad entre sesiones \\
\bottomrule
\end{tabular}
\end{table}
...
La aplicación móvil complementa el almacenamiento embebido con persistencia local de sesión y perfil. En la app, \texttt{SecureStore} conserva la sesión de Firebase y mantiene el acceso del usuario entre aperturas de la aplicación. El mismo mecanismo se usa para guardar la sesión activa de viaje, de modo que el seguimiento pueda recuperarse o cerrarse de forma coherente si la aplicación se reinicia. Para perfiles vehiculares, la aplicación utiliza FileSystem de Expo: guarda el JSON del perfil activo y su metadato asociado en el directorio local de documentos, evitando descargar el mismo perfil cada vez que se entra al flujo OBD2.

En la nube, Firestore conserva el documento principal del usuario en la colección \texttt{users}. Allí se guarda información estructurada como datos de perfil, vehículo, configuración, estado de conexión en tiempo real, dashboard, eficiencia y alertas. Los viajes se almacenan como subcolección bajo \texttt{users}, asociada al identificador del usuario autenticado. Firebase Storage se utiliza para perfiles vehiculares descargables, organizados por identificador de perfil, versión y archivo \texttt{profile.json}. La aplicación obtiene esos perfiles cuando necesita aplicar una configuración específica al ESP32-S3.

Las Cloud Functions aparecen en esta sección únicamente como servicios consumidos por la aplicación. En el flujo actual, la app llama funciones para resolver el perfil vehicular a partir del VIN, obtener información de descarga de perfil y registrar la aplicación de un perfil al dispositivo. Sin embargo, el código de esas funciones no forma parte del repositorio local revisado, por lo que esta tesis no las describe como implementación interna del proyecto, sino como parte de la integración remota utilizada por la aplicación.

En conjunto, el almacenamiento de AutoSense está diseñado para no perder la recolección por falta de conexión móvil y, al mismo tiempo, permitir sincronización remota cuando la app y Firebase están disponibles. El ESP32-S3 resguarda la evidencia básica en binario, la aplicación mantiene continuidad de sesión y perfiles, y Firebase organiza los datos de usuario, vehículo y recorridos para consulta posterior.
...
Si la app no encuentra el dispositivo en el tiempo de búsqueda configurado, cambia a un estado de error visible para el usuario. Si ya existe un dispositivo previamente conectado, intenta reconectarse usando el identificador guardado localmente. Esta decisión mejora la experiencia de uso porque evita repetir exploraciones completas cuando el mismo teléfono se conecta al mismo adaptador en sesiones sucesivas.
...
\node[seqbox, fill=gray!12] (user) {Usuario abre\\Tiempo real};
\node[seqbox, fill=green!12, below=of user] (scan) {App busca\\AutoSense OBD2};
\node[seqbox, fill=blue!10, below=of scan] (info) {\texttt{GET\_DEVICE\_INFO}\\y \texttt{READ\_VIN}};
\node[seqbox, fill=cyan!12, below=of info] (profile) {Firebase resuelve\\perfil vehicular};
\node[seqbox, fill=yellow!16, below=of profile] (transfer) {App transfiere\\perfil por BLE};
\node[seqbox, fill=orange!15, below=of transfer] (apply) {ESP32-S3 aplica\\perfil y sondeo};
\node[seqbox, fill=purple!10, below=of apply] (stream) {\texttt{START\_STREAM}\\telemetría 1 s};
\node[seqbox, fill=teal!12, below=of stream] (sync) {App muestra datos\\y sincroniza estado};
...
\addlinespace
Storage & Avatares por usuario y perfiles vehiculares de solo lectura autenticada. & Escritura arbitraria de archivos o modificación directa de perfiles publicados. \\
\bottomrule
\end{tabular}
\end{table}
